...
class AdzunaScraper:
    ...
    def __init__(self, app_id: str = None, app_key: str = None, use_kafka: bool = True):
        ...
        self.kafka_topic = "indeed-offres"   # même topic pour compatibilité pipeline
        self.producer    = None
    ...
    def save_to_json(self, offers: List[Dict], out_dir: str = "/opt/airflow/scrapers/data"):
        ...
        path = f"{out_dir}/indeed_offres_{ts}.json"   # nom compatible pipeline existant
    ...
    def save_to_minio(self, offers: List[Dict]):
        try:
            ...
            client.put_object(
                "bronze", f"indeed-linkdin/offres_{ts}.json",
                io.BytesIO(data), len(data), "application/json"
            )
            logger.info(f"📦 Minio: bronze/indeed-linkdin/offres_{ts}.json")
        except Exception as e:
